...
if(!empty($_GET['iddetmp'])) {
  ...
  $idvtr = $_GET['idvtr'];
  $sql_detform = " SELECT iddetmp, idvtr, vtrimg, vtrtipo, codmil, nomeguerra, grad, img, odomsaida, odomentr, horasaida, horaentr, destino, detmp_status, obs, num_rai FROM detmapa, vtr, pessoas  WHERE iddetmp = $iddetmp AND idvtr = vtrid AND idpessoa = codmil ORDER BY iddetmp DESC ";
    ...
    <div class="col-sm mt-2">
      ...
      <form action='model.php' method='POST' class='row gx-2 gy-2 text-center'>
        <div class="row text-center">
          <div class="form-floating col-sm-1" id="vtr">
            <img src="../veiculos/vtrimg/<?=$vtrimg; ?>" alt="<?=$vtrtipo; ?>" width="52" height='58'>
          </div>
          <div class='col-sm form-floating '> 
            <select class='form-select' name='idvtr' onchange='atualizaOdomVtr(this.value); mostrarFotoVtr(this.value)' required>
          ...
          <div class="form-floating col-sm-1" id="img">
            <img src="../pessoas/pessoas_img/<?=$pessoaimg?>" class='rounded-2' width='46' height="56" alt="<?=$nomeguerra;?>">
          </div>
          <div class='form-floating col-sm'>
            <select class='form-select' name='pessoa' onchange="mudarFotoMotorista(this.value)" required>
          ...
          <div class="input-group" id='odomentr'>
            ...
            <input type="number" class="form-control"  name="odomsaida" value="<?=$odomsaida;?>" placeholder="KM saída">
            <span class="input-group-text">KM Chegada:</span>
            <input type="number" class="form-control"  name="odomentr" value="<?=$odomentr;?>" min="<?=$odomsaida;?>" placeholder="kM chegada">
          </div>

          <div class="input-group">
            <span class="input-group-text"><i class='far fa-hourglass'></i></span>
            <span class="input-group-text">Hora saída:</span>
            <input type="time" class="form-control" name="horasaida"  value="<?=$horasaida;?>">
            <span class="input-group-text">Hora chegada:</span>
            <input type="time" id="horaentr" class="form-control"  name="horaentr" value="<?=$horaentr;?>" >
            <button type="button" class="btn btn-info" onclick="atualizaHoraChegada()"><i class='far fa-hourglass'></i></button>
          </div>
          <div class="col-sm form-floating"> 
            <select class="form-select" name="destino" id="destino" onchange="mostrarCampoNumRai(this.value)"  required>
          ...
          <div class="col-sm-2 form-floating" id="rai">
            <input type='int' class='form-control input-group-text' name='num_rai' value=<?php if($num_rai) echo $num_rai; else echo 0 ;?>>
            <label for='num_rai'>Num RAI:</label>
          </div>

        <div class="col-sm form-floating"> 
          <select class="form-select" name="status" id="status" onchange="mudaCorStatus(this.value)" required>
        ...
        <div class="form-floating"> 
          <textarea rows="5" cols="50" class="form-control" name="obs" placeholder="obs"><?=$obs;?></textarea>
...
<script>
  function atualizaHoraChegada() {  
    var d = new Date();
   document.getElementById("horaentr").value = d.getHours() + ":" + d.getMinutes();
